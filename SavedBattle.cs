...
namespace SaveBattle
{
    ...
    public static class SavedBattle
    {
        ...
        public static async void Load()
        {
            ...
            {
                FileLog.Log($"[{Debug.GetCurrentMethodName()}] Failed to load saved battle data! {ex}");
            }
        }
        ...
        public static async void Save(StageManagerType managerType = StageManagerType.Map)
        {
            ...
            try
            {
                ...
                foreach (var unit in Units)
                {
                    ...
                    foreach (var buf in statuses)
                    {
                        if (buf.bufType != KeywordBuf.None)
                        {
                            if (unitSaveData.Bufs.ContainsKey(buf.bufType))
                            {
                                unitSaveData.Bufs[buf.bufType] = buf.stack;
                            }
                            else
                            {
                                unitSaveData.Bufs.Add(buf.bufType, buf.stack);
                            }
                        }
                    }
            ...
            {
                FileLog.Log($"[{Debug.GetCurrentMethodName()}] Failed to save battle data! {ex}");
            }
        }
    }
}
